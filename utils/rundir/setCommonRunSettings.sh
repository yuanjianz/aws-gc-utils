...
Diag_Monthly="1"
Diag_Frequency="010000"
Diag_Duration="010000"
...
if [[ ${MassFlux_Entry} != "missing" ]]; then
    ...
    if [[ ${C180_Entry} != "missing" ]]; then
        #### Dynamically scale mass flux and courant number for only GEOS-IT for now
    ...
    # No need to have this requirement if no regridding is performed
    if [[ ${lowest_res} != ${highest_res} ]]; then
